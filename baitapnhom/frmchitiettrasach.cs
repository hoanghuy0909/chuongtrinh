...
namespace baitapnhom
{
    public partial class frmchitiettrasach : Form
    {
        ...
        {
            txtthahtien.Text = DAO.GetFieldValues("select sum(DATEDIFF(day,c.ngaythue,f.ngaytra)*(a.dongiathue)+i.tienphat) from sachtruyen a join chitietthuesach b on a.masach=b.masach join thuesach c on c.mathue=b.mathue join chitiettrasach d on d.masach=a.masach join trasach f on f.matra=d.matra join vipham i on i.mavipham=d.mavipham where c.mathue='%"+txtmathue.Text+"%'");
        }
    }
}
